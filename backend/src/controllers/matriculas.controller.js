...
export const getMatriculaCompleta = async (req, res) => {
  try {
    ...
    if (!categoriaRows.length) {
      console.error("Categoría no encontrada para matrícula", id, "categoria_id:", matricula.categoria_id);
      return res.status(404).json({ error: "Categoría no encontrada para esta matrícula" });
    }
